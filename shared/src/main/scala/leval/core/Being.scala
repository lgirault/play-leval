...
(owner : StarIdx,
 ...
 hasAlreadyDrawn : Boolean = false //for Helios rule
  ...
  def -( s :Suit) : Being = copy(resources = resources - s)
  def +( kv : (Suit, Card)) : Being  = copy(resources = resources + kv)
...
(id : PlayerId,
 ...
 hand : Set[Card]){
  ...
  def +( i : Int) : Star = copy(majesty = majesty + i)
  def -( i : Int) : Star = copy(majesty = majesty - i)

  def +(c : Card) : Star = copy(hand = hand + c)
  def -(c : Card) : Star = copy(hand = hand - c)
  def ++(cs : Iterable[Card]) : Star = copy(hand = hand ++ cs)
  def --(cs : Iterable[Card]) : Star = copy(hand = hand -- cs)
